...
class MultipleCanonicalForm(forms.Form):
    ...
    fruit_default = MultipleCanonicalEnumField(
        Fruit, required=False, empty_value=(Fruit.APPLE,)
    )
...
class MultiCanonicalFormTests(TestCase):
    ...
    def test_coerces_with_canonical_name(self):
        form = MultipleCanonicalForm(
            {"num": ["one", "two"], "fruit": ["apple", "peach"]}
        )
        self.assertTrue(form.is_valid())
    ...
    def test_ordered_enums_validate_by_canonical_name(self):
        # Can't pass in display name or index.
        display_form = CanonicalForm(
            {"num": ["uno", "dos"], "fruit": ["apple", "peach"]}
        )
...
class MultipleIndexForm(forms.Form):
    ...
    num_default = MultipleIndexEnumField(
        Number, required=False, empty_value=(Number.ONE, Number.TWO)
    )
